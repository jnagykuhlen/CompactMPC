...
public class TcpMultiPartyNetworkSession : IMultiPartyNetworkSession
{
    ...
    {
        var remotePartySessions = await TcpTwoPartyNetworkSession.EstablishAsync(localParty, localEndPoint, remoteEndPoints);
        return new TcpMultiPartyNetworkSession(localParty, remotePartySessions);
    }

    public void Dispose()
    {
        foreach (var remotePartySession in _remotePartySessions)

...
public abstract class PackedArray<T> : IReadOnlyList<T>, IWriteOnlyList<T>
{
    ...
    {
        var numberOfBytes = RequiredBytes(numberOfElements, elementsPerByte);
    ...
    public IEnumerator<T> GetEnumerator() => Enumerable.Range(0, Length).Select(i => this[i]).GetEnumerator();
    ...
    {
        var byteIndex = index / elementsPerByte;
        var bitsPerElement = 8 / elementsPerByte;
        var shift = bitsPerElement * (index % elementsPerByte);

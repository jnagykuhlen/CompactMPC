...
public class NaorPinkasObliviousTransfer : ObliviousTransfer
{
    ...
    {   
        ...
        var listOfCs = new Quadruple<BigInteger>();
        var listOfExponents = new Quadruple<BigInteger>();

        Parallel.For(0, 4, i =>
        {
            listOfCs[i] = GenerateGroupElement(out var exponent);
        ...
        var writeCsTask = WriteGroupElements(channel, listOfCs);
        var readDsTask = ReadGroupElements(channel, numberOfInvocations);

        var listOfExponentiatedCs = new Quadruple<BigInteger>();
        Parallel.For(1, 4, i =>
        {
        ...
        var maskedOptions = new Quadruple<Message>[numberOfInvocations];
        Parallel.For(0, numberOfInvocations, j =>
        {
            maskedOptions[j] = new Quadruple<Message>();
            var exponentiatedD = BigInteger.ModPow(listOfDs[j], alpha, _parameters.P);
            var inverseExponentiatedD = Invert(exponentiatedD);
    ...
    protected override async Task<Message[]> InternalReceiveAsync(IMessageChannel channel, QuadrupleIndexArray selectionIndices, int numberOfInvocations, int numberOfMessageBytes)
    {
        ...
        var listOfCs = new Quadruple<BigInteger>(await ReadGroupElements(channel, 4));
        ...
        var listOfBetas = new BigInteger[numberOfInvocations];
        var listOfDs = new BigInteger[numberOfInvocations];
        ...
        var writeDsTask = WriteGroupElements(channel, listOfDs);
        var readMaskedOptionsTask = ReadOptions(channel, numberOfInvocations, numberOfMessageBytes);

        var listOfEs = new BigInteger[numberOfInvocations];
        Parallel.For(0, numberOfInvocations, j =>
        {
    ...
    private BigInteger Invert(BigInteger groupElement) =>
        BigInteger.ModPow(groupElement, _parameters.Q - 1, _parameters.P);

    private Task WriteGroupElements(IMessageChannel channel, IReadOnlyList<BigInteger> groupElements)
    {
        var message = new Message(groupElements.Count * _parameters.GroupElementSize);
    ...
    private async Task<BigInteger[]> ReadGroupElements(IMessageChannel channel, int numberOfGroupElements)
    {
        ...
        var groupElements = new BigInteger[numberOfGroupElements];
        for (var i = 0; i < numberOfGroupElements; ++i)
    ...
    {
        var message = new Message(4 * numberOfInvocations * numberOfMessageBytes);
        for (var j = 0; j < numberOfInvocations; ++j)
        {
            for (var i = 0; i < 4; ++i)
    ...
    private async Task<Quadruple<Message>[]> ReadOptions(IMessageChannel channel, int numberOfInvocations, int numberOfMessageBytes)
    {
        ...
        var options = new Quadruple<Message>[numberOfInvocations];
        for (var j = 0; j < numberOfInvocations; ++j)
        {
    ...
    {
        var query = new Message(_parameters.GroupElementSize + 2 * sizeof(int))

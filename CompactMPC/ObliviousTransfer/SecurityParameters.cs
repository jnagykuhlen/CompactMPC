...
public partial record SecurityParameters(BigInteger P, BigInteger Q, BigInteger G, int GroupElementSize, int ExponentSize)
{
    public static SecurityParameters CreateDefault2048Bit()
    {
        // Recommendation from RFC 3526, 2048-bit MODP group, id 14
        var p = ParseHexString(
            """
            0
        ...
        var q = (p - 1) / 2;
        var g = (BigInteger)4;
        const int groupElementSize = 2048 / 8;
    ...
    private static BigInteger ParseHexString(string hexString) =>
        BigInteger.Parse(RemoveWhitespaceRegex().Replace(hexString, ""), NumberStyles.AllowHexSpecifier);
